// ============================================================
// UPDATED: Your existing Razorpay webhook script with email functionality
// ============================================================
// This replaces your current doPost function
// Just copy-paste this into your existing Google Apps Script
// ============================================================
...
function doPost(e) {
  try {
    ...
    var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("CALA Payment Sheet");
    ...
    // 5. Build test link
    var testLink = CONFIG.BASE_TEST_URL +
                   '?uuid=' + CONFIG.TEST_UUID +
                   '&payment_id=' + payment.id;

    // 6. Send email with invoice and test link
    var emailStatus = "No";
    if (payment.email && payment.email.trim() !== '') {
      try {
        sendPaymentEmail(paymentData, testLink);
        emailStatus = "Yes";
      } catch (emailError) {
        emailStatus = "Failed";
    ...
    errorSheet.appendRow([new Date(), "ERROR", error.message]);
...
// ============================================================
// SEND EMAIL WITH INVOICE (HTML) AND TEST LINK
// ============================================================
function sendPaymentEmail(paymentData, testLink) {
...
IMPORTANT - TO UNLOCK THE TEST:
1. Click the link above
2. You MUST use these exact credentials:
   Email: ${paymentData.email}
   Phone: ${paymentData.contact}
3. These must match EXACTLY (case-sensitive)
4. Valid for ONE assessment only
...
CuraGo Team`;
  ...
  // HTML version - CLEAN PROFESSIONAL TEMPLATE (NO COLORS, NO EMOJIS)
  var htmlBody = `
<!DOCTYPE html>
<html>
<head>
...
<body>
  <div class="email-container">
    ...
    <div class="header">
      ...
      <p>CALA 1.0 Assessment Access</p>
    </div>

    <!-- Content -->
    <div class="content">
      <p class="greeting">Dear Customer,</p>
      <p>Thank you for your payment. Your CALA 1.0 (Clinical Anxiety Loop Assessment) access has been confirmed.</p>
      ...
      <div class="invoice-box">
        ...
        <table class="invoice-table">
          ...
          <tbody>
            <tr>
              <td>
                <strong>CALA 1.0 Assessment</strong><br>
                <span style="color: #666; font-size: 12px;">Clinical Anxiety Loop Assessment Tool</span>
              </td>
      ...
      <div class="important-box">
        ...
        <p style="margin: 15px 0; line-height: 1.7;">To access your assessment, you must authenticate using the credentials provided during payment. Enter the following details exactly as shown:</p>

        <div class="credentials-box">
          <p><span class="label">Email Address</span> ${paymentData.email}</p>
          <p><span class="label">Phone Number</span> ${paymentData.contact}</p>
        </div>

        <p style="margin: 20px 0 10px 0; font-weight: 600;">Important Notes:</p>
        <ul style="margin: 10px 0; padding-left: 20px; line-height: 1.9;">
          <li>Credentials are case-sensitive and must match exactly</li>
          <li>Assessment link is valid for one-time use only</li>
          <li>Expected completion time: 10-15 minutes</li>
  ...
  GmailApp.sendEmail(
    paymentData.email,
    'Your CuraGo CALA 1.0 Assessment - Invoice & Access Link',
    plainBody,
    {
      htmlBody: htmlBody,
      name: 'CuraGo Team'
...
// ============================================================
// TEST FUNCTION - Run this to test email
// ============================================================
function testEmail() {
  var testPaymentData = {
    timestamp: new Date(),
    orderId: 'order_test123',
    paymentId: 'pay_test456ABC',
  ...
  sendPaymentEmail(testPaymentData, testLink);

  Logger.log('✅ Test email sent to: ' + testPaymentData.email);
