...
export function FloatingButtons({ onBookNow, hideButtons = false }: FloatingButtonsProps) {
  const [showWhatsAppDialog, setShowWhatsAppDialog] = useState(false);

  const handleBookNowClick = () => {
    trackButtonClick('Book Now', 'floating_cta');
    onBookNow();
  };
        ...
        <>
          ...
          >
            <Button
              ...
              size="lg"
              className="bg-[#25D366] hover:bg-[#20BA5A] text-white shadow-2xl rounded-full px-8 h-14 transition-all duration-300 hover:scale-105 hover:shadow-[#25D366]/50"
            >
              Book Now
            </Button>
          </motion.div>
